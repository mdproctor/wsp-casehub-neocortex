...
Why does this matter to an end user? Because derived knowledge is where agents stop feeling stupid. An agent that knows Alice has a child named Bob but can't infer that Bob has a parent named Alice is doing dictionary lookup, not reasoning. The derived edge rules let the graph maintain these inferences transparently — the cognitive subsystems (mental model, strategy, narrative) see a complete graph without having to compute the inverse relationships themselves.
